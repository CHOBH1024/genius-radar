  ...
  const handleExport = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        const file = new File([blob], `GeniusRadar_${surveyName.replace(/\s+/g, '_')}.png`, { type: 'image/png' });
        ...
        if (mode === 'download' || !navigator.share || !navigator.canShare || !navigator.canShare({ files: [file] })) {
          ...
          try {
            await navigator.share({
              title: '나의 Genius Radar 결과',
              text: `나는 "${resultData.persona}" 천재! 당신도 해봐요 👉 genius-radar.vercel.app`,
              files: [file]
            });
          } catch (shareError) {
            // 공유 취소/실패 시 다운로드로 대체
        ...
        >
          ...
          <div className="z-10 w-full flex flex-col items-center h-full justify-between">
            ...
            <div className="w-full flex justify-between items-center px-4">
              <div className="text-slate-400 text-2xl font-bold">@GeniusRadar</div>
              <div className="text-slate-400 text-2xl">genius-radar.vercel.app</div>
            </div>
          </div>
        </div>
      </div>
